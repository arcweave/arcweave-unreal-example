...
inline bool ends_with_new(std::string const & value, std::string const & ending)
{
...
std::any ArcscriptVisitor::visitMention_attributes(ArcscriptParser::Mention_attributesContext *ctx) {
  ...
  if (valueNode != NULL) {
    ...
    if ((strvalue.rfind("\"", 0) == 0 && ends_with_new(strvalue, "\"")) ||
         strvalue.rfind("'", 0) == 0 && ends_with_new(strvalue, "'")) {
